...
def _populate_counter_family(
    ...
    if without_counter_suffixes:
        family.samples = [
            sample._replace(name=sample.name[: -len("_total")]) if sample.name.endswith("_total") else sample
...
class _CustomCollector:
    ...
    def _translate_metric(
        ...
        label_keys, label_rows, values, exemplars = self._collect_data_points(metric.data, scope_attrs)
    ...
    def _collect_data_points(
        ...
        reserved_scope_labels = _RESERVED_SCOPE_LABELS & set(scope_attrs.keys())
        ...
        for point in metric_data.data_points:
            ...
            for from_scope, (key, value) in chain(
                ...
                if not from_scope and label in reserved_scope_labels:
                    ...
                    continue
                keys.add(label)
                label_values_by_key.setdefault(label, {})[key] = self._check_value(value)

            labels: dict[str, str] = {}
            for label, original_key_values in label_values_by_key.items():
                labels[label] = ";".join(
                    original_key_values[original_key] for original_key in sorted(original_key_values)
                )
            rows.append(labels)
            ...
            exemplars.append(_first_exemplar(getattr(point, "exemplars", None)))
