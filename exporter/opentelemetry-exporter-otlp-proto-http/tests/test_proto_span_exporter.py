...
class TestOTLPSpanExporter(unittest.TestCase):
    ...
    def test_429_is_retryable(self, mock_post):
        ...
        with self.assertLogs(level=WARNING) as warning:
            ...
            self.assertIn(
                "Transient error TOO_MANY_REQUESTS encountered while exporting span batch, retrying in",
                warning.records[0].message,
            )
    ...
    def test_retry_after_header_http_date_is_honored(self, mock_post):
        ...
        resp.headers["Retry-After"] = retry_at.strftime("%a, %d %b %Y %H:%M:%S GMT")
        mock_post.return_value = resp
        with self.assertLogs(level=WARNING) as warning:
            exporter.export([BASIC_SPAN])
            message = warning.records[0].message
            self.assertIn("retrying in", message)
            reported = float(message.split("retrying in ")[1].rstrip("s.").rstrip("s"))
        ...
        with self.assertLogs(level=WARNING) as warning:
            ...
            for record in warning.records:
                if "retrying in" in record.message:
                    reported_backoffs.append(float(record.message.split("retrying in ")[1].rstrip("s.").rstrip("s")))
...
class TestCommonRetryHelpers(unittest.TestCase):
    ...
    def test_parse_retry_after_seconds(self):
        self.assertEqual(
            _parse_retry_after_header(self._response(429, {"Retry-After": "120"})),
            120.0,
        )
    ...
    def test_parse_retry_after_http_date(self):
        ...
        delay = _parse_retry_after_header(self._response(503, {"Retry-After": header}))
    ...
    def test_parse_retry_after_http_date_in_past(self):
        ...
        header = retry_at.strftime("%a, %d %b %Y %H:%M:%S GMT")
        self.assertEqual(
            _parse_retry_after_header(self._response(503, {"Retry-After": header})),
            0.0,
        )

    def test_parse_retry_after_invalid(self):
        self.assertIsNone(_parse_retry_after_header(self._response(429, {"Retry-After": "not-a-date"})))
