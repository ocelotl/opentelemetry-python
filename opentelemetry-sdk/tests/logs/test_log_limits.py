...
class TestLoggerProviderLimits(unittest.TestCase):
    ...
    def test_default_provider_uses_default_attribute_count_limit(self):
        ...
        attributes = {f"key_{index}": index for index in range(_DEFAULT_OTEL_ATTRIBUTE_COUNT_LIMIT + 5)}
    ...
    def test_programmatic_limits_bound_attribute_count(self):
        provider = LoggerProvider(limits=LogRecordLimits(max_log_record_attributes=2))
    ...
    def test_programmatic_limits_bound_attribute_value_length(self):
        provider = LoggerProvider(limits=LogRecordLimits(max_log_record_attribute_length=3))
    ...
    def test_programmatic_limits_via_global_fallbacks(self):
        provider = LoggerProvider(limits=LogRecordLimits(max_attributes=1, max_attribute_length=2))
